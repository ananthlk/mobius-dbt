...
with base as (
    select
        ...
        k.psychologist_pct,
        k.aprn_pct,
        l.market_share_claims,
        l.leakage_claims
    from {{ ref('org_kpis_v2') }} k
    left join {{ ref('org_leakage_v2') }} l
        on  l.org_slug    = k.org_slug
        and l.period_year = k.period_year
    where k.org_type              != 'OTHER'
      and k.panel_per_clinician    is not null
...
to_national as (
    select org_type, 'type_only' as peer_group,
        ...
        avg(market_share_claims)                                   as mean_market_share,
        approx_quantiles(market_share_claims, 4)[offset(1)]        as p25_market_share,
        approx_quantiles(market_share_claims, 4)[offset(2)]        as p50_market_share,
        approx_quantiles(market_share_claims, 4)[offset(3)]        as p75_market_share,
        avg(leakage_claims)                                        as mean_leakage,
        approx_quantiles(leakage_claims, 4)[offset(1)]             as p25_leakage,
        approx_quantiles(leakage_claims, 4)[offset(2)]             as p50_leakage,
        approx_quantiles(leakage_claims, 4)[offset(3)]             as p75_leakage
    from base
